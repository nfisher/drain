...
const (
	FormatJSONL        = parseFormatJSONL
	FormatParquet      = parseFormatParquet
	DefaultBatchSize   = defaultParseBatchSize
	DefaultBatchMaxAge = defaultParseBatchMaxAge
	JSONLContentType   = parseJSONLContentType
	ParquetContentType = parseParquetContentType
)
...
type Sink interface {
	Write(context.Context, Output) error
...
func ValidateSinkOptions(format string, batchSize int, batchMaxAge time.Duration) error {
	switch format {
...
func NewSink(_ context.Context, stdout io.Writer, opts SinkOptions) (Sink, error) {
	if opts.Now == nil {
		opts.Now = time.Now
	}
	if opts.Prefix == "" {
...
func (w *jsonlStreamWriter) Write(_ context.Context, output Output) error {
	if w.excludeSource {
		output.SourceKind = ""
		output.SourceName = ""
	}
...
func newPartJSONLWriter(store ObjectStore, runID string, batchSize int, batchMaxAge time.Duration, includeParameters, excludeSource bool, now func() time.Time) *partJSONLWriter {
...
func (w *partJSONLWriter) Write(ctx context.Context, output Output) error {
...
func newPartParquetWriter(store ObjectStore, runID string, batchSize int, batchMaxAge time.Duration, includeParameters, excludeSource bool, now func() time.Time) *partParquetWriter {
...
func (w *partParquetWriter) Write(ctx context.Context, output Output) error {
	if w.writer != nil && w.rowsInPart > 0 && !w.now().Before(w.partStarted.Add(w.batchMaxAge)) {
		if err := w.closePart(); err != nil {
			return err
		}
...
func appendParseParquetRow(builder *array.RecordBuilder, output Output, includeParameters, excludeSource bool) {
...
func newParseObjectStore(prefix string, s3Flags S3Options) (ObjectStore, error) {
	if strings.HasPrefix(prefix, "s3://") {
		return NewS3Store(prefix, s3Flags)
	}
	if strings.Contains(prefix, "://") {
		return nil, fmt.Errorf("unsupported output prefix scheme in %q", prefix)
	}
	return LocalStore{Prefix: prefix}, nil
}
