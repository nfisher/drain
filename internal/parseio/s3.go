...
func ResolveS3Config(options S3Options) (S3Config, error) {
	...
	useSSL, err := boolFromFlagEnvOrFile(options.UseSSL, options.UseSSLFile, "s3-use-ssl-file", useSSLDefault, []string{"S3_USE_SSL"}, []string{"S3_USE_SSL_FILE"})
	if err != nil {
		return S3Config{}, err
	}
	pathStyle, err := boolFromFlagEnvOrFile(options.PathStyle, options.PathStyleFile, "s3-path-style-file", true, []string{"S3_PATH_STYLE"}, []string{"S3_PATH_STYLE_FILE"})
	if err != nil {
		return S3Config{}, err
	}
	region, err := stringFromFlagEnvOrFile(options.Region, options.RegionFile, []string{"S3_REGION", "AWS_REGION", "AWS_DEFAULT_REGION"}, []string{"S3_REGION_FILE", "AWS_REGION_FILE", "AWS_DEFAULT_REGION_FILE"})
	if err != nil {
		return S3Config{}, err
	}
	accessKeyID, err := stringFromFlagEnvOrFile(options.AccessKeyID, options.AccessKeyIDFile, []string{"S3_ACCESS_KEY_ID", "AWS_ACCESS_KEY_ID"}, []string{"S3_ACCESS_KEY_ID_FILE", "AWS_ACCESS_KEY_ID_FILE"})
	if err != nil {
		return S3Config{}, err
	}
	secretAccessKey, err := stringFromFlagEnvOrFile(options.SecretAccessKey, options.SecretAccessKeyFile, []string{"S3_SECRET_ACCESS_KEY", "AWS_SECRET_ACCESS_KEY"}, []string{"S3_SECRET_ACCESS_KEY_FILE", "AWS_SECRET_ACCESS_KEY_FILE"})
	if err != nil {
		return S3Config{}, err
	}
	sessionToken, err := stringFromFlagEnvOrFile(options.SessionToken, options.SessionTokenFile, []string{"S3_SESSION_TOKEN", "AWS_SESSION_TOKEN"}, []string{"S3_SESSION_TOKEN_FILE", "AWS_SESSION_TOKEN_FILE"})
	if err != nil {
		return S3Config{}, err
	}
...
func stringFromFlagEnvOrFile(valueFlag, fileFlag OptionalString, envNames, envFileNames []string) (string, error) {
	if valueFlag.Set {
...
func boolFromFlagEnvOrFile(valueFlag OptionalBool, fileFlag OptionalString, fileFlagName string, defaultValue bool, envNames, envFileNames []string) (bool, error) {
	if valueFlag.Set {
		return valueFlag.Value, nil
	}
	if fileFlag.Set {
